...
import { fetchMyPlan, UserBillingSummary } from '@/src/lib/billing-service';
...
export function Sidebar({ isCollapsed, onToggle }: SidebarProps) {
  ...
  React.useEffect(() => {
    fetchMyPlan().then(setSummary).catch(() => {});
  }, []);

  const planName = summary?.plan?.name?.toUpperCase() || 'FREE PLAN';
  const usagePercentage = summary?.usage?.products?.percentage || 0;
  const isFree = summary?.plan?.key === 'free';
      ...
      <div className="p-4 mt-auto border-t border-border/50">
        {!isCollapsed ? (
          <Link 
            ...
            className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-primary/5 hover:bg-primary/10 border border-transparent hover:border-primary/20 transition-all group overflow-hidden"
          >
            <div className="shrink-0 p-2 bg-primary/10 rounded-xl text-primary group-hover:scale-110 transition-transform">
              <Zap size={18} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-1 mb-0.5">
                <p className="text-[11px] font-black text-primary truncate">{planName}</p>
                {isFree && (
                  <span className="text-[9px] font-black bg-primary text-primary-foreground px-1.5 py-0.5 rounded shadow-sm">
                    UPGRADE
                  </span>
                )}
              </div>
              <p className="text-[10px] text-muted-foreground font-bold truncate">
                {usagePercentage}% limits used
              </p>
            </div>
          </Link>
        ) : (
          <Link 
            href="/dashboard/billing" 
            className="w-12 h-12 rounded-2xl bg-primary/5 border border-transparent flex items-center justify-center text-primary hover:bg-primary/10 hover:border-primary/20 transition-all mx-auto group"
          >
